...
-- Volcando datos para la tabla sigepi.integrante_has_grupo_inve_semillero: ~20 rows (aproximadamente)
...
CREATE TABLE IF NOT EXISTS `proyecto_has_grupo_inve_semillero` (
  ...
  CONSTRAINT `FK_proyecto_has_grupo_inve_semillero_grupo_inve_semillero` FOREIGN KEY (`grupo_inve_semillero_id_grupo`) REFERENCES `grupo_inve_semillero` (`id_grupo`),
  CONSTRAINT `FK_proyecto_has_grupo_inve_semillero_proyecto` FOREIGN KEY (`proyecto_id_proyecto`) REFERENCES `proyecto` (`id_proyecto`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
-- Volcando datos para la tabla sigepi.usuario_has_proyecto: ~0 rows (aproximadamente)
